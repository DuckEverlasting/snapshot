...
// Plan for next restructure:
/*
  - Need to redo the render system again. More of a step in the direction it's already going.
  - Going to make the main canvas take up the entire workspace.
  - Will have to redo translate / zoom interpretation. Should be a lot easier to consolidate it (?)
  - Will need to think more about handling crop and transform object, and cursor parsing.
    - Probably would actually be best to draw a custom cursor on. (would be good for drawing variable circle to indicate radius)
    - Print cursor as separate layer
    - Print pixel grid
*/
